...
from gan_utils import save_args, GANModel, GenerateDataCallback, GeneratorTrainingCallback
...
class CDiscriminatorNetwork(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.trunk = nn.Sequential(*[m for m in [
            nn.Conv2d(6, 64, kernel_size=4, stride=2, padding=1),  # N, 64, 14, 14
            nn.InstanceNorm2d(64) if args.discriminator_batchnorm else None,
            nn.LeakyReLU(),
            nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1),  # N, 128, 7, 7
            nn.InstanceNorm2d(128) if args.discriminator_batchnorm else None,
            nn.LeakyReLU(),
            Reshape(-1, 128 * 7 * 7),  # N, 128*7*7
            nn.Linear(128 * 7 * 7, 1024),  # N, 1024
            nn.InstanceNorm1d(1024) if args.discriminator_batchnorm else None,
            nn.LeakyReLU(),
