...
class CDiscriminatorNetwork(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.trunk = nn.Sequential(*[m for m in [
            ...
            nn.InstanceNorm2d(512) if args.discriminator_instancenorm else None,
            nn.LeakyReLU(negative_slope=0.2),
            ...
            nn.InstanceNorm2d(512) if args.discriminator_instancenorm else None,
            nn.LeakyReLU(negative_slope=0.2),
            Reshape(-1, 512 * 4 * 8),  # N, 512 * 8 * 4
            nn.Linear(512 * 4 * 8, 1024),  # N, 1024
            nn.LeakyReLU(),
            nn.Linear(1024, 1),  # N, 1
            Reshape(-1)] if m is not None])  # N
        # nn.Conv2d(512, 1, kernel_size=(8, 16), stride=(4, 8), padding=(2, 4)),  # N, 1, 1, 1
        # nn.Sigmoid(),
        # Reshape(-1)] if m is not None])  # N
